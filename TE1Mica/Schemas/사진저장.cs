...
namespace TE1.Schemas
{
    ...
    public class 사진자료 : Dictionary<카메라구분, 사진저장>
    {
        ...
        #region 사진저장
        public void SaveImage(그랩장치 장치, 검사정보 검사) => SaveImage(장치.구분, 장치.MatImage(), 검사.일시);
        public void SaveImage(카메라구분 카메라, Mat image, DateTime 시간)
        {
        ...
        {
            String path = Path.Combine(Global.환경설정.원본보관폴더, Utils.FormatDate(시간, "{0:yyyy-MM-dd}"), 카메라.ToString());
            Common.DirectoryExists(path, true);
            return path;
        }
        public String OriginImageFile(DateTime 시간, 카메라구분 카메라)
        {
            String path = OriginImagePath(시간, 카메라);
            String file = SaveImageFileName(시간, 사진형식.Png);
        ...
        public String CopyImageFile(DateTime 시간, 카메라구분 카메라)
        {
            if (!this.ContainsKey(카메라)) return String.Empty;
            return CopyImageFile(시간, 카메라, this[카메라].사본유형);
        }
        public String CopyImageFile(DateTime 시간, 카메라구분 카메라, 사진형식 형식)
        {
            String path = CopyImagePath(시간, 카메라);
            String file = SaveImageFileName(시간, 형식);
            if (String.IsNullOrEmpty(path) || String.IsNullOrEmpty(file)) return String.Empty;
            return Path.Combine(path, file);
        }
        public static String SaveImageFileName(DateTime 시간, 사진형식 형식) =>
            $"{Utils.FormatDate(시간, "{0:HHmmss}")}.{형식.ToString()}";
        #endregion
    }
}
